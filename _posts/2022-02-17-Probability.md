...
The weak law of large number: for i.i.d. samples $$X_i$$, $$\displaystyle\bar X=\frac{1}{n}\sum_{k=1}^{n}{X_k}$$, $$\displaystyle D[\bar X]=\frac{σ^2}{n}$$. Using *Chebyshev's*, $$X$$ -> $$\bar X$$, $$a$$ -> $$\epsilon$$, then $$\displaystyle P\{\vert\bar X-μ\vert\geq \epsilon\}\leq\frac{1}{n}\frac{\sigma^2}{\epsilon^2}$$ goes to zero when $$n$$ goes to infinity.
