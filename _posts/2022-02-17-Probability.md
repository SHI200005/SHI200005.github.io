...
$$P(A|B)=\frac{P(A,B)}{P(B)}$$ 很重要！
### 全概率公式与Bayes 公式

1. $$P(B_i|A)=\frac{P(A|B_i)P(B_i)}{\sum_{j=1}^{n}P(A|B_j)P(B_j)}$$
...
以二维随机变量为例 $$F(x,y)=P\{(X≤x)∩(Y≤y)\}=P\{X≤x, Y≤y\}$$
 - 连续型随机变量联合概率密度 $$F(x,y)=\int_{-∞}^y{f(u,v)dudv}$$, $$\frac{\partial^2{F(x,y)}}{\partial x\partial y}=f(x,y)$$.
...
连续型随机变量边缘概率密度：$$f_X(x)=\int_{-∞}^{∞}{f(x,y)dy}$$.
...
- P.S. 并不是所有概率分布都存在有限的方差！例如 $$P(x)=\frac{1}{\pi}\frac{\gamma}{(x-a^2)+\gamma^2}(-\infty<x<\infty)$$，也称为 Lorentz-Cauchy distribution 就没有。
...
对概率分布做**傅里叶变换**（参见[复变函数](https://shi200005.github.io/2022/02/15/Complex-Functions/)）得到 *the characteristic function* $$\tilde p(k)=\langle e^{-ikx}\rangle=\int dxp(x)e^{-ikx}$$，then moments of the PDF around any point $$x_0$$ can be generated by expanding $$e^{ikx_0}\tilde p(k)=\displaystyle\sum_{n=0}^\infty\frac{(-ik)^n}{n!}\langle(x-x_0)^n\rangle$$。

### The cumulant generating function

（补充，参见 Kardar 2.2）

is $$ln\tilde p(k)$$，我们称把这个东西对 $$k$$ 的展开项是 *cumulants*，Kardar 在书中用 $$\langle x^n\rangle_c$$ 表示。Why do we care about cumulants? 对于图形法背后的数学，在书中图形法的后一页解释得很清楚了，不再赘述。
...
高斯分布 $$p(x)=\frac{1}{\sqrt{2\pi\sigma^2}}exp[-\frac{(x-\lambda)^2}{2\sigma^2}]$$ 的优良品质是 $$\langle x\rangle_c=\lambda$$, $$\langle x^2\rangle_c=\sigma^2$$, $$\langle x^3\rangle_c=\langle x^4\rangle_c=...=0$$，也就是任何阶中心矩都可以用均值和方差两个参数表示。
...
相关系数：$$ρ_{XY}=\frac{Cov(X,Y)}{\sqrt{D(X)}\sqrt{D(Y)}}$$.

协方差时二阶混合中心矩。协方差矩阵的定义...性质：[证明：协方差矩阵是半正定矩阵](https://blog.csdn.net/qcyfred/article/details/71598815)。半正定的概念参见[线性代数](https://shi200005.github.io/2021/09/30/Linear-Algebra/)。
...
*Markov's inequality*: $$X$$ non-negative, for every $$a>0$$, $$P[X\geq a]\geq\frac{E[X]}{a}$$.
...
Note that 核心关系 $$D[\frac{1}{n}\sum_{k=1}^{n}{x_k}]=\frac{σ^2}{n}$$, which means $$\sigma$$~$$N^{-1/2}$$. Plug in *Chebyshev's inequality*, we have *For all $$\epsilon>0$$ and all $$\delta>0$$ there exists a positive integer $$n_0$$ such that for all $$n\geq n_0$$*, $$P[\vert M_n-m\vert\geq\epsilon]\leq\delta$$. 
...
Sum $$X=\displaystyle\sum_{i=1}^{N}x_i$$, $$x_i$$ are i.i.d. from a distribution. We have $$\langle X^n\rangle_c=N\langle x^n\rangle_c$$. Construct $$y=\frac{X-N\langle x\rangle_c}{\sqrt N}$$, so $$\langle y^n\rangle_c\propto N^{1-n/2}$$. As $$N\rightarrow\infty$$, **only the second cumulant survives**, and the PDF for $$y$$ converges to the **normal distribution**, $$\displaystyle\lim_{N→\infty}p(y=\frac{\displaystyle\sum_{i=1}^N x_i-N\langle x \rangle_c}{\sqrt N})=\frac{1}{\sqrt{2\pi\langle x^2\rangle_c}}exp(-\frac{y^2}{2\langle x^2\rangle_c})$$. 
