...
无偏性：虽然得到的估计值与真实值不同，但**估计值的期望**与真实值一样。反复将这一估计量使用多次，就“平均”来说其偏差为零 -> 无系统误差。e.g., 样本 $$k$$ 阶**原点矩**是对总体 $$k$$ 阶原点矩的**无偏估计**；样本**二阶中心矩**就是对总体均值的**有偏估计**。

有效性：如果在样本容量 $$n$$ 相同的情况下，某一**无偏**估计量的方差更小，就更有效。e.g., 算术均值比加权均值有效（证明过程中用了**方幂不等式**，不难证明）。罗-克拉美不等式——达到方差下界的无偏估计量，没用过，课件里有例子，以后用到再说。
