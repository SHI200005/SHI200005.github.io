...
**自感**：用于线圈时，回路可能有外来电流、或回路形状及周围磁介质发生变化，此时穿过回路自身的磁通量随之改变，从而在回路中激发感应电动势。$$\displaystyle\Sigma=-L\frac{dI}{dt}$$ 自感电动势将反抗回路中电流的改变。结合 $$\displaystyle\Sigma=-\frac{d\Phi_\text{tot}}{dt}$$，其中 $$\Phi_\text{tot}=N\Phi$$，于是 $$\displaystyle L=N\frac{\Phi}{I}$$。空心密绕圆柱长螺线管 $$B=\mu_0nI$$，于是 $$\Phi=BA=\mu_0nIA$$，推出 $$L=\mu_0n^2Al$$。如果中间是磁导率为 $$\mu$$ 的铁磁质（见下文），则 $$L=\mu n^2Al$$。
