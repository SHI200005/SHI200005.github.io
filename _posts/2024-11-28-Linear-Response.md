...
A time-invariant linear system: input $$\vec x(t)$$, response function $$h(t)$$ (causality requires $$h(t)=0$$ for $$t<0$$), output $$\vec y(t)$$. Denote the linear operator $$L$$ where $$\vec y(t)=L[\vec x(t)]$$.  From Dirac's delta function, $$\displaystyle\vec x(t)=\int_{-\infty}^t \vec x(t^\prime)\delta(t-t^\prime)dt^\prime$$. Then apply $$L$$ on both sides, $$\displaystyle L[\vec x(t)]=L[\int_{-\infty}^t \vec x(t^\prime)\delta(t-t^\prime)dt^\prime]$$. Note that $$L$$ operates on $$t$$, and the system is time-invariant, so  $$\displaystyle L[\vec x(t)]=\int_{-\infty}^t \vec x(t^\prime)L[\delta(t-t^\prime)]dt^\prime$$. Denote $$h(t)=L[\delta(t)]$$, then we get the **convolution theorem** $$\displaystyle\vec y(t)=\int_{-\infty}^t \vec x(t^\prime)h(t-t^\prime)dt^\prime$$.
